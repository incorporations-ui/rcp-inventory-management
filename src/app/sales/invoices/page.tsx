...
export default function InvoicesPage() {
  ...
  async function voidInvoice(inv: any) {
    if (inv.dispatch_status === 'dispatched') {
      toast.error('Cannot void a dispatched invoice. Raise a Sales Return instead.')
      setVoidItem(null); return
    }
    setVoiding(true)
    try {
      // 1. Restore stock for each invoice line
      const { data: lines } = await supabase.from('invoice_lines').select('sku_id, units').eq('invoice_id', inv.id)
      for (const line of lines ?? []) {
        await supabase.rpc('update_stock_master', { p_sku_id: line.sku_id, p_delta: line.units })
          .then(({ error }) => { if (error) console.warn('Stock restore warning:', error.message) })
      ...
      // 2. Void invoice
      await supabase.from('invoices').update({ dispatch_status: 'voided', payment_status: 'voided' }).eq('id', inv.id)
      // 3. Revert PL to finalized (items stay packed, but invoice is gone)
      if (inv.packing_list_id) {
        await supabase.from('packing_lists').update({ status: 'finalized' }).eq('id', inv.packing_list_id)
      }
      // 4. Revert SO to approved so invoice can be regenerated
      await supabase.from('sales_orders').update({ status: 'approved' }).eq('id', inv.so_id)
      toast.success(`Invoice ${inv.invoice_number} voided. SO reset to Approved — regenerate invoice from the Packing List.`)
      setVoidItem(null)
      loadData()
    } catch (e: any) {
  ...
  async function printInvoice(inv: any) {
    ...
    try {
      const { data: lines } = await supabase
        .from('invoice_lines').select('*, skus(display_name, sku_code, hsn_code)')
        .eq('invoice_id', inv.id).order('id')
      generateAndPrintPDF({
        docType: 'TAX_INVOICE',
        docNumber: inv.invoice_number,
        docDate: formatDate(inv.invoice_date),
        soNumber: inv.sales_orders?.so_number,
        customer: {
          name: inv.customers?.name,
          address_line1: inv.customers?.address_line1,
          city: inv.customers?.city,
          state: inv.customers?.state,
          gstin: inv.customers?.gstin,
        },
        lines: (lines ?? []).map((l: any) => ({
          description: l.skus?.display_name ?? l.skus?.sku_code ?? '—',
          sku_code: l.skus?.sku_code,
          hsn_code: l.skus?.hsn_code ?? '',
          qty: l.units,
          unit_price: Number(l.unit_price),
          gst_rate: Number(l.gst_rate),
          line_amount: Number(l.line_amount),
          line_gst: Number(l.line_gst),
        })),
    ...
    <AppLayout>
      <PageGuard>
          ...
          <div className="page-header">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                {filtered.length} invoices · Total: {formatCurrency(filtered.reduce((s, i) => s + Number(i.grand_total), 0))}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <SearchInput value={search} onChange={setSearch} />
                ...
                <table>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      <tr key={inv.id} className={inv.dispatch_status === 'voided' ? 'opacity-50 line-through' : ''}>
        ...
        <ConfirmDialog
          ...
          onConfirm={() => voidItem && voidInvoice(voidItem)}
          title="Void Invoice"
          message={`Void Invoice ${voidItem?.invoice_number}? Stock will be restored and the SO will revert to Approved so you can regenerate the invoice from the Packing List. This cannot be undone.`}
